...
contract dWork {
  ...
  mapping(address => Checklist[]) public dContracts;  // Payer/Contractor to respective ongoing Contracts
  mapping(address => uint) public trustToken;  // Track how much trust each person has
  ...
  function addContract(address payable payer, string memory contractName) public {
  ...
  function getContract(uint index) public returns(Checklist){
    return dContracts[msg.sender][index];
  }
  function addTrustToken() public {

  }
  function removeTrustToken() public {
